import{f as _,r as w,j as e,h as V,C as G,M as Y,i as D,k as Q,g as K,l as X,X as Z,A as j}from"./index-Cbj6GPdr.js";import{s as i,u as J,P as W,a as ee}from"./Playbooks-B_s0gvKP.js";/**
...
correct scope.
`,Be=/^([|>])([+-]?)(\d*)([+-]?)$/,E=/^(\s*)([A-Za-z0-9_.-]+)\s*:\s?(.*)$/,A=/^(\s*)-\s+(.*)$/;function x(n){return n.length-n.trimStart().length}function T(n,s){if(n.length<2||!n.startsWith(s)||!n.endsWith(s))return!1;const t=n.slice(1,-1);if(s==="'"){for(let a=0;a<t.length;a+=1)if(t[a]==="'"){if(t[a+1]==="'"){a+=1;continue}return!1}return!0}for(let a=0;a<t.length;a+=1){if(t[a]==="\\"){a+=1;continue}if(t[a]==='"')return!1}return!0}function $(n){return T(n,'"')?n.slice(1,-1).replace(/\\(["\\])/g,"$1"):T(n,"'")?n.slice(1,-1).replace(/''/g,"'"):n}function B(n){if(n.startsWith('"')||n.startsWith("'"))return n;const s=n.search(/(^|\s)#/);return s===-1?n:n.slice(0,s)}function I(n){const s=B(n.trim()).trim();return $(s)}function qe(n){const s=B(n.trim()).trim();if(T(s,'"')||T(s,"'"))return $(s);if(s==="true")return!0;if(s==="false")return!1;if(s!==""){const t=Number(s);if(!Number.isNaN(t))return t}return s}function Le(n){let s="",t=0;for(const a of n){if(a===""){t+=1;continue}s===""?s=a:(s+=t>0?`
`.repeat(t):" ",s+=a),t=0}return s}function Ue(n,s,t){return t==="strip"?n:t==="keep"?n+`
`.repeat(s+1):n===""?"":`${n}
`}function ze(n,s,t,a,d){const c=[];let l=s;for(;l+1<n.length;){const r=n[l+1];if(r.trim()===""){c.push(""),l+=1;continue}if(x(r)<=t)break;c.push(r),l+=1}let h=c.length-1;for(;h>=0&&c[h]==="";)h-=1;const o=c.length-1-h,u=c.slice(0,h+1),m=u.reduce((r,p)=>p===""?r:Math.min(r,x(p)),Number.MAX_SAFE_INTEGER),g=u.map(r=>r===""?"":r.slice(Math.min(m,x(r)))),v=a==="|"?g.join(`
`):Le(g);return{value:Ue(v,o,d),endIndex:l}}function He(n){const s=n[2]||n[4];return s==="-"?"strip":s==="+"?"keep":"clip"}function Ve(n,s){for(let t=s+1;t<n.length;t+=1){const a=n[t];if(!(a.trim()===""||a.trimStart().startsWith("#")))return a}}function Ge(n){return n.replace(/^\uFEFF/,"").replace(/\r\n?/g,`
`)}function Ye(n){const s=Ge(n);if(!/^---[ \t]*(\n|$)/.test(s))return{data:{},content:s};const t=s.split(`
