...
const RAW_CATALOGUE: WorkflowPreset[] = [
  ...
  // ── Document — produce something a reader keeps ───────────────────────────────
  {
    ...
    injector: [
      ...
      "- Build a small, self-contained static HTML mockup (no framework, no build step) under",
      "  design/<date>-<slug>/mockup.html. Link this app's real client/theme/tokens.css by",
      "  relative path (e.g. ../../client/theme/tokens.css) and wrap the visual content in",
      "  id=\"mockup-root\" instead of hand-copying token hex values — it reads honestly rather",
      "  than as a generic wireframe, and never drifts from the real palette.",
      "- Screenshot it with `npm run design-shot -- --input design/<date>-<slug>/mockup.html",
      "  --out design/<date>-<slug>/desktop.png --theme dark --viewport desktop`, then again",
      "  with `--viewport mobile` for mobile.png (390x740). The script always uses whatever",
      "  Playwright version this repo has installed, so there is nothing to pin by hand.",
      "- Commit the HTML and both PNGs to design/<date>-<slug>/ on a real branch and push it.",
      "  Build the permanent raw.githubusercontent.com URL from the owner, repository, branch,",
      "  and path.",
      "- Publish the writeup wherever the reader will actually see it:",
      "  - Notion: create or update a dated page under the correct parent with the ntn CLI,",
      "    embedding each screenshot as an ordinary Markdown image reference — Notion resolves",
      "    external URLs without any upload step. Verify the images resolved by reading the",
      "    page back.",
      "  - A tracking GitHub issue or PR: comment directly with `gh issue comment <n>",
      "    --body-file <path>` (or `gh pr comment`), embedding the raw.githubusercontent.com",
      "    URLs as Markdown images. If the branch has no local checkout, `gh api",
      "    repos/<owner>/<repo>/contents/<path> -X PUT -F content=@<base64-file> -f",
      "    branch=<branch>` uploads a file directly — use -F, not -f, for the @file field or",
      "    the literal string \"@<path>\" is sent instead of the file's contents.",
      ...
      "- Report the branch, the raw URLs, and wherever the writeup was published (Notion page",
      "  URL, issue/PR comment URL, or both).",
    ].join("\n"),
  },
  {
    id: "docs-preview",
